...
class UrlBuilderFromSearch {
  ...
  getOrsUrl() {
    ...
    if (!this.isOrs) {
      infoNav(
        `Invalid search: ${this.search}; sending user to help page`,
        "getOrsUrl"
      );
      return errorUrl;
    }
  ...
  async getOrLawUrl() {
    ...
    let specialSession =
      orLawReader != "hein" ? this.getSpecialSession() : null; // only worry about special session if not using Hein
    let newOrLawRequest = new OrLawRequest(
      year,
      chapter,
      specialSession,
      orLawReader
...
class OrLawRequest {
  ...
  returnDataErrors() {
    ...
    if (this.hasSpecialSession) errMsg.push(this.ssErrors());
    return errMsg
      .filter((exists) => {
        exists;
      })
      .join("/n"); // I'm not quite sure what I'm doing with the error handling. It doesn't really get back to user in any way at this point.
  }
  yearErrors() {
    ...
    if (this.year < minYear) {
      return `${
        this.isOrLeg ? "Legislature website" : "HeinOnline"
      } does not have Oregon Law records pre-${minYear}. '${
        this.year
      }' is too early.`;
    }
  ...
  async orLegUrl() {
    ...
    if (orLawOrLegLookup[this.year] != undefined) {
      let orLawFileName = orLawOrLegLookup[this.year].replace(
        /~/,
        "000" + this.chapter
      ); // creates url suffix
      ...
      warnBG(
        `Cannot find [${this.year}] in ORS lookup.`,
        "navigate.js",
        "OrLawRequest.orLegUrl"
      );
      return "";
    }
  }
...
const buildAndNavigateToUrls = (searchString) => {
  ...
  infoNav(
    `Received ${listOfSearches.length} search request(s): ${listOfSearches}`,
    "buildAndNavigateToUrls"
  );
  listOfSearches.forEach(async (aSearch) => {
    ...
    if (search.url && search.url?.length > 0) {
      ...
      warnBG(
        `URL is ${search.url}; something's wrong. Navigating to ${errorUrl}`
      );
