...
window.addEventListener("load", async () => {
  ...
  const setInputCodeAndHtmlColors = (colorObject) => {
    ...
    try {
      ...
      infoOptions(
        `Set colors for example & option stylesheets: '${JSON.stringify(
          colorObject
        )}'`,
        "cssLoadColors"
      );
      ...
      warnOptions(
        `Error setting stylesheet colors from '${JSON.stringify(
          colorObject
        )}.' Error: ${error}`,
        "cssLoadColors"
      );
  ...
  const doHideSaveAndCancel = (doHide) => {
    doHide
      ? buttonsDiv.classList.add("invisibility")
      : buttonsDiv.classList.remove("invisibility");
    if (doHide) {
      doHideImportMenu(true);
    }
  ...
  userEditableColorsList.forEach((cssVarName) => {
    if (optionsFrame.getElementById(cssVarName)) {
      ...
      warnOptions(
        `Couldn't load : ${cssVarName}`,
        "addListeners(cssElementArray)"
      );
    }
  });
  ...
  const saveToUserBrowserStorage = async (userData) => {
    try {
      ...
      infoOptions(
        `Successful save of ${JSON.stringify(userData)}`,
        "saveToUserBrowserStorage"
      );
    } catch (error) {
